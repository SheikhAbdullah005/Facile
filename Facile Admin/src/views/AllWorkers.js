...
import { Container, Row, Col, Card, Button, ButtonGroup, CardHeader, CardBody } from "shards-react";
...
class Customer extends React.Component {
 ...
 componentDidMount(){
   axios.get(`http://localhost:3005/api/fetch_verified_vendors`)
     .then(res => {
   ...
   return this.state.customer.map(elem=>{
     return <tr
       onChange={() => {
         ...
         this.changeCustomer(elem.user_address);
         this.changeCustomer(elem.user_cnic);
         this.changeCustomer(elem.user_image);
         this.changeCustomer(elem.user_verification_status);
       }}
     >
      <td>
       {elem.user_id}
 ...
 render() {
   return (
      ...
      <Row noGutters className="page-header py-4">
        <PageTitle sm="4" title="All Verified Workers" subtitle="Workers" className="text-sm-left" />
      </Row>
        ...
        <Col>
          <Card small className="mb-4">
            <CardHeader className="border-bottom">
              <h6 className="m-0">Verified Workers</h6>
            </CardHeader>
