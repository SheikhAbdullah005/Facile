...
public class CustomerInProgressJobDetailsActivity extends AppCompatActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    private String phoneNumber, jobId,vendorId,currentJobInitialPrice;
    ...
    WalletDeductionHandler subtracAmount;

    FirebaseAuth mAuth;
    String abc="abbb",phoneNum,customerPhone;
    StorageReference storageReference;
    DatabaseReference databaseReference;
    String currentUserID,totalPayment,serviceID,vendorName,serviceName,cityName,lat,lon,email;
    ...
    protected void onCreate(Bundle savedInstanceState) throws  NullPointerException {
        ...
        toast = Toast.makeText(getApplicationContext(), "Paid successfully",
                Toast.LENGTH_SHORT);
        misc = new Misc(this);
        ...
        //abc = mAuth.getInstance().getCurrentUser().getUid();
        //For Checking if request is accepted then gone the visibility of cancel button

    }

    private void getVender(String vendorId) {
        Ion.with(this)
                .load("GET",misc.ROOT_PATH+"user_profile/"+vendorId)
                .asString()
                .withResponse()
                    ...
                    public void onCompleted(Exception e, Response<String> result) {
                        if(e != null) {
                            ...
                            try {
                               ...
                               // misc.showToast(jsonObject.getString("job_status"));
//                               // misc.showToast("123");
//
                                if(jsonObject.getString("job_status").equalsIgnoreCase("accepted")){
                                   subtracAmount = new WalletDeductionHandler();
                                   subtracAmount.setId(customerId);
                                   subtracAmount.deductMoney();
                                    changeCartStatus();
                                    changeDailyJobStatus();
                                    cancelRequest();
                                    misc.showToast("Amount Rs: 100/- has been deducted from the wallet");
                                   // misc.showToast("Ma chudao");
                                }else{
                                    //misc.showToast("Ma chudao");
                                    changeCartStatus();
                                    changeDailyJobStatus();
    ...
    public void usersChat(){
        ...
        chat.putExtra("receiverEmail", email);
...
//////////////payment Dialog
    public void paymentBox(final String price){
        ...
        final TextView card_button = dialog.findViewById(R.id.card);
                                    ...
                                    card_button.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View v) {
                                            ...
                                            if(checkAmount(price)){
                                                misc.showToast("Insufficient balance");
                                            } else {
                                                complete.setVisibility(View.VISIBLE);
                                                updateWallet(price);
                                                dialog.dismiss();
                                                misc.showToast("You paid Rs: " + price + "/- succesfully");
                                                receivedByCard();
                                            }//toast.show();
                                        }
                                    });
        ...
        //onBackPressed();
       // toast.show();
        //intent_this = new Intent(getApplicationContext(),CustomerInProgressJobDetailsActivity.class);
    ...
    private void updateWallet(final String rs) {
            Ion.with(this)
                    .load(misc.ROOT_PATH+"get_wallet/"+customerId)
                    .asString()
                    .withResponse()
                        ...
                        public void onCompleted(Exception e, Response<String> result) {
                            if(e != null) {
                                ...
                                try {
                                    ...
                                    int previousAmount = Integer.valueOf(previous_amount) - Integer.valueOf(rs);
                                    String rss = String.valueOf(previousAmount);
                                    JsonObject jsonObjectnew = new JsonObject();
                                    jsonObjectnew.addProperty("newAmountString", rss);
                                    Ion.with(getApplicationContext())
                                            .load("PUT",misc.ROOT_PATH + "update_amount/" + customerId)
    ...
    private boolean checkAmount(final String rs){
        Ion.with(this)
                .load(misc.ROOT_PATH+"get_wallet/"+customerId)
                .asString()
                .withResponse()
                    ...
                    public void onCompleted(Exception e, Response<String> result) {
                        if (e != null) {
                            ...
                            try{
                                ...
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    JSONObject jsonObject = (JSONObject) jsonArray.get(i);
                                    amountGot = Integer.valueOf(jsonObject.getString("amount"));
                                }
                            }catch (JSONException e1) {
                                e1.printStackTrace();
                            }
        ...
        if(amountGot<Integer.valueOf(rs)){
            return true;
        }else{
            return false;
        }
